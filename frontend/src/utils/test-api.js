...
const log = (title, data, error = false) => {
  console.log(
    `%c ${title} `,
    `background: ${error ? '#f44336' : '#4CAF50'}; color: white; font-weight: bold; border-radius: 3px;`, 
    data
  );
};
...
async function runAllTests() {
  console.log('%c API TESTING STARTED ', 'background: #2196F3; color: white; font-weight: bold; font-size: 14px; padding: 5px;');
  ...
  console.log('%c API TESTING COMPLETE ', 'background: #2196F3; color: white; font-weight: bold; font-size: 14px; padding: 5px;');
}
...
if (typeof window !== 'undefined') {
  ...
  button.style = 'position: fixed; bottom: 20px; right: 20px; z-index: 9999; padding: 10px; background: #2196F3; color: white; border: none; border-radius: 4px; cursor: pointer;';
  ...
  console.log('%c API Test Script Loaded! ', 'background: #2196F3; color: white; font-weight: bold; font-size: 14px; padding: 5px;');
  console.log('Click the button in the bottom right to run tests, or call runAllTests() from the console.');
}
